...
def quick_private_company_valuation(company_data: PrivateCompanyData, normalized_earnings: float, capitalization_rate: float=0.2) -> float:
...
def calculate_marketability_discount(revenue: float, profitability: str='Average', reporting_quality: str='Compiled') -> float:
    """Quick marketability discount calculation"""
    analyzer = DiscountPremiumAnalyzer()
    characteristics = {'revenue': revenue, 'profitability': profitability, 'financial_reporting_quality': reporting_quality, 'growth_prospects': 'Average', 'management_quality': 'Average'}
...
def estimate_control_premium(ownership_percentage: float) -> float:
    ...
    if 'control_premium' in result:
        ...
        return -result['minority_discount']
    else:
        return 0

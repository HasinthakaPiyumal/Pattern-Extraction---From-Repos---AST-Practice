...
class DBnomicsTab(BaseTab):
    ...
    def create_navigation_panel(self, *args, **kwargs):
        ...
        try:
            ...
            dpg.add_listbox([], tag='providers_listbox', width=380, num_items=6, callback=self.on_provider_selected)
            dpg.add_separator()
            dpg.add_text('2. SELECT DATASET', color=self.BLOOMBERG_YELLOW)
            dpg.add_listbox([], tag='datasets_listbox', width=380, num_items=5, callback=self.on_dataset_selected)
            dpg.add_separator()
            dpg.add_text('3. SELECT SERIES', color=self.BLOOMBERG_YELLOW)
            dpg.add_listbox([], tag='series_listbox', width=380, num_items=5, callback=self.on_series_selected)
            dpg.add_separator()
        ...
        try:
            with operation('load_providers'):
                ...
                if response.status_code == 200:
                    ...
                    dpg.set_frame_callback(dpg.get_frame_count() + 1, lambda: self.update_status(f'Failed to load providers: {response.status_code}'))
        except Exception as e:
            error('Error in load_providers_thread', context={'error': str(e)}, exc_info=True)
            dpg.set_frame_callback(dpg.get_frame_count() + 1, lambda: self.update_status(f'Provider error: {str(e)}'))
    ...
    def _process_providers_response(self, data, from_cache=False):
        ...
        try:
            ...
            dpg.set_frame_callback(dpg.get_frame_count() + 1, lambda: self.update_providers_list(provider_items, cache_info))
        except Exception as e:
        ...
        try:
            with operation(f'load_datasets_{provider_code}'):
                ...
                if response.status_code == 200:
                    ...
                    error('Failed to load datasets', context={'provider_code': provider_code, 'status_code': response.status_code})
                    dpg.set_frame_callback(dpg.get_frame_count() + 1, lambda: self.update_status(f'Failed to load datasets: {response.status_code}'))
        except Exception as e:
            error('Error in load_datasets_thread', context={'provider_code': provider_code, 'error': str(e)}, exc_info=True)
            dpg.set_frame_callback(dpg.get_frame_count() + 1, lambda: self.update_status(f'Dataset error: {str(e)}'))
    ...
    def _process_datasets_response(self, data, provider_code, from_cache=False):
        ...
        try:
            ...
            cache_info = ' (cached)' if from_cache else ''
            dpg.set_frame_callback(dpg.get_frame_count() + 1, lambda: self.update_datasets_list(dataset_items, cache_info))
        except Exception as e:
    ...
    def on_dataset_selected(self, sender, app_data, *args, **kwargs):
        try:
            ...
            if dpg.does_item_exist('series_listbox'):
                dpg.configure_item('series_listbox', items=[])
            thread = threading.Thread(target=self.load_series_thread, args=(self.current_provider, dataset_code), daemon=True)
            thread.start()
        except Exception as e:
        ...
        try:
            with operation(f'load_series_{provider_code}_{dataset_code}'):
                ...
                if response.status_code == 200:
                    ...
                    error('Failed to load series', context={'provider_code': provider_code, 'dataset_code': dataset_code, 'status_code': response.status_code})
                    dpg.set_frame_callback(dpg.get_frame_count() + 1, lambda: self.update_status(f'Failed to load series: {response.status_code}'))
        except Exception as e:
            error('Error in load_series_thread', context={'provider_code': provider_code, 'dataset_code': dataset_code, 'error': str(e)}, exc_info=True)
            dpg.set_frame_callback(dpg.get_frame_count() + 1, lambda: self.update_status(f'Series error: {str(e)}'))
    ...
    def _process_series_response(self, data, provider_code, from_cache=False):
        ...
        try:
            ...
            for series in series_data:
                ...
                self.series_list.append({'code': code, 'name': name, 'full_id': full_id})
                display_item = f'{code} - {name[:45]}'
                series_items.append(display_item)
            cache_info = ' (cached)' if from_cache else ''
            dpg.set_frame_callback(dpg.get_frame_count() + 1, lambda: self.update_series_list(series_items, cache_info))
        except Exception as e:
    ...
    def on_series_selected(self, sender, app_data, *args, **kwargs):
        try:
            ...
            thread = threading.Thread(target=self.load_series_data_thread, args=(selected_series['full_id'], selected_series['name']), daemon=True)
            thread.start()
        except Exception as e:
                ...
                if response.status_code == 200:
                    ...
                    if series_docs:
                        ...
                        if 'period' in first_series and 'value' in first_series:
                            ...
                            for i in range(min(len(periods), len(values))):
                                if values[i] is not None:
                                    observations.append({'period': periods[i], 'value': values[i]})
                        ...
                        if observations:
                            ...
                            self.current_data = {'series_id': full_series_id, 'series_name': series_name, 'observations': observations}
                            dpg.set_frame_callback(dpg.get_frame_count() + 1, lambda: self.display_current_data())
                            return
                warning('No data found for series', context={'series_id': full_series_id})
                dpg.set_frame_callback(dpg.get_frame_count() + 1, lambda: self.update_status('No data found'))
        except Exception as e:
            error('Error in load_series_data_thread', context={'series_id': full_series_id, 'error': str(e)}, exc_info=True)
            dpg.set_frame_callback(dpg.get_frame_count() + 1, lambda: self.update_status(f'Data load error: {str(e)}'))
    ...
    def create_single_table(self, observations, series_name, *args, **kwargs):
        try:
            with operation(f'create_table_{len(observations)}_rows'):
                table_id = dpg.add_table(header_row=True, borders_innerH=True, borders_outerH=True, height=480, scrollY=True, parent='single_data_area')
                ...
                for slot_index in range(6):
                    ...
                    if dpg.does_item_exist(slot_area_id):
                        ...
                        if data_point is None:
                            dpg.add_text(f'SLOT {slot_index + 1}: Empty', color=self.BLOOMBERG_GRAY, parent=slot_area_id)
                        else:
                            series_name = data_point['series_name']
                            observations = data_point['observations']
                            dpg.add_text(f'SLOT {slot_index + 1}: {series_name[:30]}', color=self.BLOOMBERG_YELLOW, parent=slot_area_id)
    ...
    def create_slot_table(self, observations, parent_id, *args, **kwargs):
        try:
            table_id = dpg.add_table(header_row=True, borders_innerH=True, borders_outerH=True, height=180, scrollY=True, parent=parent_id)
        ...
        try:
            ...
            with operation('export_data'):
                ...
                if not df.empty:
                    ...
                    info(f'Data exported to: {filename}', context={'rows': len(df), 'filename': filename})

...
class CohereLLM(BaseLLM):
    ...
    def __init__(self, name: Optional[str]=None, cohere_api_key: Optional[str]=None):
    ...
    def _initialize_client(self, cohere_api_key: Optional[str]=None):
        ...
        try:
            ...
            raise ImportError("Please install Cohere to use CohereLLM. You can install it with: `pip install 'semantic-router[cohere]'`")
        ...
        try:
            ...
            raise ValueError(f'Cohere API client failed to initialize. Error: {e}') from e
        return client
    ...
    def __call__(self, messages: List[Message]) -> str:
        ...
        try:
            completion = self._client.chat(model=self.name, chat_history=[m.to_cohere() for m in messages[:-1]], message=messages[-1].content)
...
class CustomFormatter(colorlog.ColoredFormatter):
    ...
    def __init__(self):
        super().__init__('%(log_color)s%(asctime)s %(levelname)s %(name)s %(message)s', datefmt='%Y-%m-%d %H:%M:%S', log_colors={'DEBUG': 'cyan', 'INFO': 'green', 'WARNING': 'yellow', 'ERROR': 'red', 'CRITICAL': 'bold_red'}, reset=True, style='%')
...
class PostgresIndexRecord(BaseModel):
    ...
    def __init__(self, **data) -> None:
        ...
        if len(clean_route) > 255:
            raise ValueError(f"The cleaned route name '{clean_route}' exceeds the 255 character limit.")
    ...
    def to_dict(self) -> Dict:
        ...
        :rtype: Dict
        """
        return {'id': self.id, 'vector': self.vector, 'route': self.route, 'utterance': self.utterance}
...
class OllamaEncoder(DenseEncoder):
    ...
    def __init__(self, name: Optional[str]=None, score_threshold: float=0.5, base_url: str | None=None):
    ...
    def _initialize_client(self, base_url: str):
        ...
        try:
            ...
            raise ImportError("The 'ollama' package is not installed. Install it with: pip install 'semantic-router[ollama]'")
...
class MistralEncoder(LiteLLMEncoder):
    ...
    def __init__(self, name: str | None=None, mistralai_api_key: str | None=None, score_threshold: float=0.4):
        ...
        super().__init__(name=name, score_threshold=score_threshold, api_key=mistralai_api_key)
...
class GoogleEncoder(DenseEncoder):
    ...
    def __init__(self, name: Optional[str]=None, score_threshold: float=0.75, project_id: Optional[str]=None, location: Optional[str]=None, api_endpoint: Optional[str]=None):
    ...
    def _initialize_client(self, project_id, location, api_endpoint):
        ...
        try:
            ...
            raise ImportError('Please install Google Cloud and Vertex AI libraries to use GoogleEncoder. You can install them with: `pip install google-cloud-aiplatform vertexai-language-models`')
        ...
        try:
            ...
            raise ValueError(f'Google AI Platform client failed to initialize. Error: {err}') from err
        return client

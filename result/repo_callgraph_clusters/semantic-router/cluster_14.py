...
@pytest.mark.parametrize('dense_encoder_cls,sparse_encoder_cls,input_type', [(encoder, sparse_encoder, input_type) for encoder in [MockSymmetricDenseEncoder, MockAsymmetricDenseEncoder] for sparse_encoder in [MockSymmetricSparseEncoder, MockAsymmetricSparseEncoder] for input_type in ['queries', 'documents']])
class TestHybridRouter:

    def test_encode(self, dense_encoder_cls, sparse_encoder_cls, input_type, routes, mocker):
        encoder = dense_encoder_cls(name='Dense Encoder')
        sparse_encoder = sparse_encoder_cls(name='Sparse Encoder')
        router = HybridRouter(encoder=encoder, sparse_encoder=sparse_encoder, routes=routes)
        ...
        if isinstance(encoder, AsymmetricDenseMixin):
            ...
            dense_encode_documents_spy = mocker.spy(dense_encoder_cls, 'encode_documents')
        if isinstance(sparse_encoder, AsymmetricSparseMixin):
            sparse_encode_queries_spy = mocker.spy(sparse_encoder_cls, 'encode_queries')
            sparse_encode_documents_spy = mocker.spy(sparse_encoder_cls, 'encode_documents')
    ...
    @pytest.mark.asyncio
    async def test_async_encode(self, dense_encoder_cls, sparse_encoder_cls, input_type, routes, mocker):
        encoder = dense_encoder_cls(name='Dense Encoder')
        sparse_encoder = sparse_encoder_cls(name='Sparse Encoder')
        router = HybridRouter(encoder=encoder, sparse_encoder=sparse_encoder, routes=routes)
        ...
        if isinstance(encoder, AsymmetricDenseMixin):
            ...
            dense_encode_documents_spy = mocker.spy(dense_encoder_cls, 'aencode_documents')
        if isinstance(sparse_encoder, AsymmetricSparseMixin):
            sparse_encode_queries_spy = mocker.spy(sparse_encoder_cls, 'aencode_queries')
            sparse_encode_documents_spy = mocker.spy(sparse_encoder_cls, 'aencode_documents')
        test_query = ['test query']

...
def run(use_gui=True, runs=1):
    ...
    env = SumoEnvironment(net_file='sumo_rl/nets/double/network.net.xml', single_agent=False, route_file='sumo_rl/nets/double/flow.rou.xml', out_csv_name=out_csv, use_gui=use_gui, num_seconds=86400, yellow_time=3, min_green=5, max_green=60)
    fixed_tl = False
    agents = {ts_id: TrueOnlineSarsaLambda(env.observation_spaces(ts_id), env.action_spaces(ts_id), alpha=1e-09, gamma=0.95, epsilon=0.05, lamb=0.1, fourier_order=7) for ts_id in env.ts_ids}
...
def run(use_gui=False, episodes=50):
    ...
    env.reset()
    agents = {ts_id: TrueOnlineSarsaLambda(env.observation_spaces[ts_id], env.action_spaces[ts_id], alpha=0.0001, gamma=0.95, epsilon=0.05, lamb=0.1, fourier_order=7) for ts_id in env.agents}
    for ep in range(1, episodes + 1):
        ...
        if fixed_tl:
            ...
            while env.agents:
                ...
                for ts_id in next_obs.keys():
                    agents[ts_id].learn(state=obs[ts_id], action=actions[ts_id], reward=r[ts_id], next_state=next_obs[ts_id], done=terminated[ts_id])
                    obs[ts_id] = next_obs[ts_id]
    env.close()
...
class SumoEnvironmentPZ(AECEnv, EzPickle):
    ...
    def step(self, action):
        ...
        if not self.action_spaces[agent].contains(action):
            raise Exception('Action for agent {} must be in Discrete({}).It is currently {}'.format(agent, self.action_spaces[agent].n, action))

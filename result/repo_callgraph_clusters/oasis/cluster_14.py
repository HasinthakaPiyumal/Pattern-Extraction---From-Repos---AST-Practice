...
def create_user_node(tx, user_id, action_info, created_at):
    query = 'MERGE (u:User {user_id: $user_id}) SET u += $action_info, u.created_at = $created_at'
    tx.run(query, user_id=user_id, action_info=action_info, created_at=created_at)

def create_follow_relationship(tx, user_id, follow_id, timestamp):
    query = 'MERGE (u1:User {user_id: $user_id}) MERGE (u2:User {user_id: $follow_id}) CREATE (u1)-[:FOLLOWS {timestamp: $timestamp}]->(u2)'

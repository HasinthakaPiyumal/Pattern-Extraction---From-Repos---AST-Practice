...
def small_dataset():
    ...
    embeddings = torch.randn(33, 10)
    labels = torch.tensor([0, 1, 2] * 11)
...
def test_ewc_single_batch_edge_case(simple_model, small_dataset):
    ...
    """
    device = 'cpu'
    ewc = EWC(simple_model, small_dataset, device=device, ewc_lambda=100.0)
...
def test_ewc_various_batch_sizes():
    ...
    class SimpleModel(nn.Module):
        ...
        def forward(self, x):
            return self.fc(x)
    test_sizes = [1, 31, 32, 33, 64, 65, 100]
...
def test_ewc_loss_computation(simple_model, small_dataset):
    ...
    assert loss_normalized.item() != loss.item()
